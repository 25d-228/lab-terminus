...
describe("TransferDrawer", () => {
  it("shows newest-first progress and exposes cancel and clear boundaries", async () => {
    ...
    render(<TransferDrawer open onOpenChange={vi.fn()} onRefresh={refresh} jobs={[
      { id: "old", kind: "copy", label: "old", state: "done", done: 10, total: 10, speed: 0 },
      { id: "new", kind: "upload", label: "new", state: "active", done: 5, total: 10, speed: 2 },
    ]} />)
    ...
    expect(fetchMock).toHaveBeenCalledWith("/api/transfers/new/cancel", { method: "POST" })
