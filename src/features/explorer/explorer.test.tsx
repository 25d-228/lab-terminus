...
describe("Explorer", () => {
  it("loads an authoritative path, filters, sorts, selects, mutates, and uploads dropped files", async () => {
    ...
    await user.type(rename, "renamed.ts")
    await user.click(screen.getByRole("button", { name: "OK" }))
    await waitFor(() => {
      expect(calls.some((call) => call === "POST /api/gpu1/fs")).toBe(true)
    })
    ...
    await waitFor(() => {
      expect(calls.some((call) => call.includes("POST /api/gpu1/upload"))).toBe(true)
    })
  ...
  it("rejects a stale directory response after the active host changes", async () => {
    ...
    vi.spyOn(globalThis, "fetch").mockImplementation((input) =>
      String(input).includes("gpu1")
        ? old
        : Promise.resolve(json({ path: "/local", entries: [] })),
    )
  ...
  it("clears the file picker so the same file can be selected repeatedly", async () => {
    const calls: string[] = []
    vi.spyOn(globalThis, "fetch").mockImplementation(async (input, init) => {
      const path = String(input)
      calls.push(`${init?.method || "GET"} ${path}`)
    ...
    const { container } = render(<ExplorerHarness server={servers[0]} />)
    await screen.findByText(/Empty folder/)
    ...
    await waitFor(() => {
      expect(calls.filter((call) => call.includes("POST /api/gpu1/upload"))).toHaveLength(1)
    })
    expect(picker.value).toBe("")

    fireEvent.change(picker, { target: { files: [file] } })
    await waitFor(() => {
      expect(calls.filter((call) => call.includes("POST /api/gpu1/upload"))).toHaveLength(2)
    })
    expect(picker.value).toBe("")
  })
})
