...
describe("registry controls", () => {
  it("validates a server and sends the typed add request", async () => {
    ...
    render(<RegistryDialog state={{ type: "server", folder: "lab" }} folders={folders} onClose={vi.fn()} onSaved={saved} />)
    await user.click(screen.getByRole("button", { name: "Save" }))
    expect(fetchMock).not.toHaveBeenCalled()
    await user.type(screen.getByLabelText("Name"), "New host")
    await user.click(screen.getByRole("button", { name: "Save" }))
    await waitFor(() => expect(fetchMock).toHaveBeenCalledWith("/api/servers", expect.objectContaining({ method: "POST" })))
  ...
  it("opens the sidebar server context menu and confirms destructive removal", async () => {
    ...
    render(<SidebarProvider><AppSidebar servers={servers} folders={folders} statuses={{}} view={{ kind: "overview" }} onViewChange={vi.fn()} onEdit={vi.fn()} onRefresh={refresh} /></SidebarProvider>)
    ...
    await waitFor(() => expect(fetchMock).toHaveBeenCalledWith("/api/servers/gpu1", expect.objectContaining({ method: "DELETE" })))
