...
export function RegistryDialog({
  ...
  useEffect(() => {
    ...
    if (state.type === "folder") {
      setFolderTitle(state.folder?.title ?? "")
      ...
      setDraft(emptyDraft(state.folder ?? folders[0]?.key ?? "lab"))
    }
  }, [folders, state])
  ...
  const save = async () => {
    ...
    try {
      if (state.type === "folder") {
        ...
        await api(
          path,
          jsonRequest(state.folder ? "PUT" : "POST", {
        ...
        await api(
          path,
          jsonRequest(state.server ? "PUT" : "POST", {
            ...draft,
    ...
    >
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Changes are saved to the active Lab Terminus configuration.
          </DialogDescription>
        </DialogHeader>
        {state?.type === "folder" ? (
          <Card size="sm">
